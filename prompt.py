...
**Pseudocódigo para la etapa de recomendación (Punto 3)**:

1. **Obtener los K vecinos más cercanos**

   * Usar `KNNCalcularDistancia.get_knn(df_ratings, usuario_objetivo, k)` para obtener un DataFrame de los K vecinos con sus distancias.

2. **Inicializar lista de recomendaciones** vacía.

3. **Para cada vecino en la lista de vecinos**:
   a. Extraer su serie de ratings (`serie_vecino`).
   b. Seleccionar las películas con `rating_vecino ≥ umbral`.
   c. Filtrar las películas que el `usuario_objetivo` **no** haya visto (`NaN`).
   d. Por cada película candidata, agregar un registro con:

   ```
   {
     'usuario_vecino': vecino,
     'pelicula': nombre_pelicula,
     'rating_vecino': valor_rating
   }
   ```

4. **Construir un DataFrame** a partir de la lista de registros.

5. **(Opcional) Agregar métricas**:

   * Contar cuántos vecinos recomiendan cada película (`veces_recomendada`).

6. **Ordenar y devolver**:

   * Por `veces_recomendada` (descendente) y luego por `rating_vecino` (descendente).

---

A continuación, el **código literal** listo para colocar en un archivo `recomendacion.py`. Está escrito con variables en español y comentarios para que se entienda cada paso. Lo puedes importar y usar directamente tras tu módulo KNN y tus funciones de distancia.
